...
class PharnetManufactContract extends Contract{

  constructor() {
    // Provide a custom name to refer to this smart contract
    super('org.pharma-network.com.pharnet.manufacturer');
  }
   ...
   * @param serialNo - unique serial number  of the drug
   * @param mfgDate -  Date on which the drug is manufactured
   * @param expDate - The last Date on which the drug can be used
   * @param companyCRN - Registration Number of the Company
 	 * @returns
 	 */
   async addDrug(ctx,drugName,serialNo,mfgDate,expDate,companyCRN){
     try{
       ...
       // check whether the regi manufacturer is invoking the tx
       if(await cid.getMSPID()  == "manufacturerMSP"){
         // Create a composite key for the company to get registered in ledger
         const productID = ctx.stub.createCompositeKey('org.pharma-network.drug',[drugName,serialNo]);

         // fetch the object from the ledger by using partial key
         let response = await ctx.stub.getStateByPartialCompositeKey('org.pharma-network.company',[companyCRN]).catch(err => console.log(err));
         let results = await this.getAllResults(response);
         let company = JSON.parse(results[0]);

         // create new drug object to be stored in the blockchain
         let drug = {
           productID: productID,
           name: drugName,
         ...
         // Convert the JSON object to a buffer and send it to blockchain for storage
         await ctx.stub.putState(productID,Buffer.from(JSON.stringify(drug)));

         // return the drug object to the user
         return drug;
       }
       // if not the manufacturer, then say invalid caller
       else{
         console.log(" *****  Caller is not a valid user ");
       }
     }
     catch(err){
...
module.exports = PharnetManufactContract;
